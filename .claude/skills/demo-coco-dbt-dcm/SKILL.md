...
## Purpose
Show attendees how Cortex Code pair-programs a real pipeline: CSVs (bundled in `data/`) → RAW (uploaded via `snow stage copy`) → dbt staging views → HOL-shape tables (matching the Cortex AI HOL #1 contract) → analytical marts. DCM owns the schemas; dbt owns every table. `.env`-gated deploy prevents accidental overwrites of an existing `PAWCORE_ANALYTICS` install.
...
| `bootstrap/01_load_raw.sql` | CREATE TABLE + COPY INTO from stage. Column mapping for `customer_reviews.csv` (order: review_id, product, region, date, review_text, rating). |
